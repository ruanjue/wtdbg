...
static inline void push_seqbank(SeqBank *sb, char *tag, int tag_len, char *seq, int seq_len){
	...
	kv_put_cuhash(sb->rdhash, ptr, sb->nseq);
	sb->nseq ++;
}

static inline u4i find_seqbank(SeqBank *sb, char *tag){ return kv_get_cuhash(sb->rdhash, tag); }

static inline u4i num_n50(u4v *lens, FILE *out){
	u8i tot, cum;
	u4i i, max, min, n50, n90, avg;
	...
	if(out){
		fprintf(out, "TOT %llu, CNT %u, AVG %u, MAX %u, N50 %u, N90 %u, Min %u", tot, (u4i)lens->size, avg, max, n50, n90, min);
		fflush(out);
	}
	return n50;
}
