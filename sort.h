...
do {	\
	...
	while(x){	\
		...
		m = s + (e - s) / 2;	\
		a = s; b = m;	\
		if((int)(is_a_greater_than_b) > 0){ swap_expr; }	\
		a = m; b = e;	\
		if((int)(is_a_greater_than_b) > 0){	\
			swap_expr;	\
			a = s; b = m;	\
		...
		while(1){	\
			a = m;	\
			while(b = i, (int)(is_a_greater_than_b) > 0) i ++;	\
			b = m;	\
			while(a = j, (int)(is_a_greater_than_b) > 0) j --;	\
			if(i < j){	\
				if(m == i) m = j;	\
				else if(m == j) m = i;	\
				a = i; b = j; \
	...
	for(i=0;i<_qsort_n;i++){	\
		...
		for(j=_qsort_n-1;j>i;j--){	\
			a = j - 1; b = j;	\
			if((int)(is_a_greater_than_b) > 0){ swap_expr; x = 1; }	\
		}	\
		if(x == 0) break;	\
	}	\
} while(0)
