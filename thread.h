...
#define thread_begin_loop(tname) tname##_params = tname->tname##_params;	\
	...
	tname##_var_j = 0;	\
	while(tname->running){\
		if(tname->state == 0){ if(tname##_var_j == 0) microsleep(1); tname##_var_j = (tname##_var_j + 1) % 10; continue; }\
		microsleep(1); \
...
//#define thread_end_loop(tname) } tname->state = 0; microsleep(1); }
...
#define thread_end_init(tname) \
		...
		while(tname->state > 0){ microsleep(1); }\
	}\
